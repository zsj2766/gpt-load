...
3. Updated `GroupManager.Initialize` to explicitly set `EffectiveConfig.RetryStrategy` from the `Group` struct's `RetryStrategy` field, ensuring the effective configuration reflects the group's specific strategy even if not present in the JSON config override.
